(function($){
    $(document).ready(function(){
        ...
        var $hello = $('#hello');//메서드 수행 후 반환해서 $hello에 배열 반환.
        //_hello = ''; //전역변수로 사용할 때 _변수명으로 정하겠다!!
        //$hello = $(''); //요소로서 사용할때 $번수명으로 정하겠다!!
        ...
        //검색한 요소 노드 중 해당 순서(작성자가 찾고자 하는 순서number type)에 따라서 찾아낼 때.
        //[index];
        //.get(index);
        //.eq(index);
        var $content1 = $content[0]; //배열 중 하나를 선택
        console.log($content1);
        $content1 = $content.get(1); //배열 중 하나를 선택
        console.log($content1);
        $content1 = $content.eq(2); //배열중 하나를 다시 배열로 반환
        console.log($content1);

        //검색한 요소 노드 중 첫번째 노드를 찾아낼 때.
        //.first();
        //$('div,contnent:first');
        //.eq(0);
        ...
        // .last();
        //$('span.contnent:last');
        //.eq(셀렉터 반환 배열의 개수.eq($spanContent.length) - 1);
        var $spanContentLast = $spanContent.last(); //배열 중 마지막 요소를 배열로 반환.
        console.log($spanContentLast);
        $spanContentLast = $spanContent.eq($spanContent.length - 1); //배열 중 마지막 요소를
        ...
        var $aNotSpan = $('a').not('span');
        console.log($aNotSpan);
        $aNotSpan = $('a:not(span)');
        console.log($aNotSpan);
        ...
        // //화면 상에서 요소가 보이는 중이라면.
        //CSS3
        //display - none - 화면 상에서 끔. 영역도 없고, 보이지도 않고
