(function($){
    $(document).ready(function(){
        ...
        */
        //Answer 1.
        $('#hello').append('<span id="my-name">제 이름은 이유진입니다.</span>');
        ...
        */
        //Answer 3.
        //$('p.move-top').prepend($('body').eq(1));
        //$('p.move-top').eq(0).before('body'); //body라는 글자가 p.move-top위에 위치함
        //$('body').eq(1).insertBefore('p.move-top');
        
        var $movetop = $('p.move-top');
        //class를 변수에 담아서 사용한다. $(' ');
        $('body').children().eq(0).before($movetop);
        //p.move-top의 텍스트가 body 맨상단으로 이동한다.

        //$($movetop).addClass('move').html('<p>위로 이동했습니다.</p>');
        $($movetop).addClass('move');
        if($movetop.hasClass('move')){
        ...
        */
        ...
        $('body').children().after($movebottom.eq(0));

        $($movebottom).addClass('move').text('아래로 이동 못했습니다 어떻게하쥐?.');
        ...
        */
        //Answer 8
        for(i=2; i <= 4; i++){
            var $list = $('ul.list-1').children().eq(i-2); 
            $list.after('<li> 리스트 '+i+' </li> ');
        }
        console.log($list);
        //$('ul.list-1').children('li').eq(0);
